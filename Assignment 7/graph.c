...
int dfsIterative(Graph* graph, Vertex* source, Vertex* destination)
{
    ...
    while(check != 1 && dequeIsEmpty(stack) != 1){
      Vertex *current = dequeBack(stack);
      dequePopBack(stack);
      current->isVisited = 1;
    ...
    for(int i = 0; i < current->numNeighbors; i++){
      if((current->neighbors[i])->isVisited != 1)
         dequePushBack(stack,current->neighbors[i]);
      }
   }
    dequeDelete(stack);
...
int bfsIterative(Graph* graph, Vertex* source, Vertex* destination)
{
    ...
    while(check != 1 && dequeIsEmpty(queue) != 1){
      Vertex *current = dequeFront(queue);
      dequePopFront(queue);
      current->isVisited = 1;
      if (current == destination){
         check = 1;
    ...
    for(int i = 0; i < current->numNeighbors; i++){
      if((current->neighbors[i])->isVisited != 1)
         dequePushBack(queue,current->neighbors[i]);
      }
   }
    dequeDelete(queue);
