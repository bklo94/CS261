...
/* Name:Brandon Lo
 * Date:2/21/17
 * Solution description: Implementation of binary search tree and the 4 functions, add node, left node, remove left node, and remove node.
 */
...
struct Node *_addNode(struct Node *cur, TYPE val)
{
	...
	//if the root is empty, set the root to the node(value is set to that node) that was just made and create the left and right to null
    if (cur == NULL) {
        assert(newNode != NULL);
        newNode->val = val;
        newNode->left = NULL;
	...
	//compare the value of the root to the one you want to insert, if it is larger set to the right, else set to the left
...
int containsBSTree(struct BSTree *tree, TYPE val)
{
    ...
    while (currentNode != NULL) {
        ...
        else if (compare(val, currentNode->val) > 0)
            currentNode = currentNode->right;
        else 
            return 1; //searches both sides of the tree until it is found
    }
    return 0; //default is not found once it is out of the while loop
}
...
TYPE _leftMost(struct Node *cur)
{
    /*write this*/
    assert(cur != NULL);
	//while the next node's left is not null keep advancing left
    while (cur->left != NULL)
        cur = cur->left;
...
struct Node *_removeLeftMost(struct Node *cur)
{
    /* Write this */
    assert(cur != NULL);
	//if the current node's left is not empty remove the left node, else remove the right node's data
    if (cur->left != NULL) {
...
struct Node *_removeNode(struct Node *cur, TYPE val)
{
    ...
    assert(val != NULL);
    assert(cur != NULL);
	//if the node is found, goes right and grabs the smallest value.
	...
	//if it is not found, it is searched for
