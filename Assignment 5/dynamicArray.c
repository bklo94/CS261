...
/* Name:Brandon Lo
 * Date:3/5/17
 * Solution description: Implementing a heapsort/adjust/remove/add
 */
...
void adjustHeap(DynamicArray* heap, int last, int position, compareFunction compare)
{
   ...
   if(right < last){
      ...
      if (compare(dyGet(heap,child), dyGet(heap,position)) == -1){
         dySwap(heap, position, child);
         adjustHeap(heap, last, child, compare);
      }
   }
...
void buildHeap(DynamicArray* heap, compareFunction compare)
{
    ...
    index = size/2-1;
    while(index >= 0){
      adjustHeap(heap,size,index,compare);
      index--;
   }
}
